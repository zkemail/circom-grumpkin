...
import { hash_to_curve, str_to_array } from "../generate_inputs";
...
describe('HashToCurve', () => {
    it('HashToField', async () => {
        ...
        const circuit = await wasm_tester(path.join(__dirname, "../../circuits/test/hash_to_curve_test.circom"));
        const circuitInputs = stringifyBigInts({ msg });
        const witness = await circuit.calculateWitness(circuitInputs);
        await circuit.checkConstraints(witness);
        const expected = hash_to_curve(msg);
        expect(witness[1]).toEqual(expected.x);
        expect(witness[2]).toEqual(expected.y);
    });
