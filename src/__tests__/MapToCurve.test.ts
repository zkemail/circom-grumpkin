...
describe('MapToCurve', () => {
    ...
    it('MapToCurve()', async () => {
        const circuit = await wasm_tester(path.join(__dirname, "../../circuits/test/map_to_curve_test.circom"))
        ...
        // Generate witness
        let us = hash_to_field(str_to_array('abc'));
        const expected = map_to_curve(us[0]);
        const circuitInputs = stringifyBigInts({
            u: us[0],
        })
